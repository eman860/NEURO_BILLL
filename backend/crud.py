...
def set_password_reset_token(db: Session, user: models.User, token: str, expiry: datetime) -> None:
...
def gst_summary(db: Session, business_id: int, month: Optional[int] = None, year: Optional[int] = None) -> schemas.GSTSummary:
    ...
    if month and year:
        q = q.filter(
            func.strftime("%m", models.Invoice.created_at) == f"{month:02d}",
            func.strftime("%Y", models.Invoice.created_at) == str(year),
        )
    invoices = q.all()
...
def invoice_aging(db: Session, business_id: int) -> List[schemas.AgingBucket]:
    """Bucket unpaid invoices by how many days old they are."""

...
# CORS – allow the frontend (served on port 8001 or 8002) to talk to this API
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:3000",
...
def forgot_password(data: schemas.ForgotPasswordRequest, db: Session = Depends(get_db)):
    """Generate a reset token and simulate sending an email."""
    user = crud.get_user_by_email(db, data.email)
    if user:
        token = secrets.token_urlsafe(32)
        expiry = datetime.utcnow() + timedelta(minutes=15)
        crud.set_password_reset_token(db, user, token, expiry)
        
        # Simulate sending email
        reset_link = f"http://localhost:8001/?reset_token={token}"
...
def reset_password(data: schemas.ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    user = crud.get_user_by_reset_token(db, data.token)
    if not user:
